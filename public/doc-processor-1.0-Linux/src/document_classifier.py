...
class DocumentClassifier:
    ...
    def __init__(self):
        ...
        logger.debug("Классификатор документов инициализирован")
    ...
    def classify(self, extracted_text: str) -> str:
        ...
        for doc_type, keywords in self.unique_keywords.items():
            for keyword in keywords:
                if keyword in text_lower:
                    logger.debug(f"Найдено ключевое слово для {doc_type}: '{keyword}'")
        ...
        if "УПД" in found_types and "СЧЕТ_ФАКТУРА" in found_types:
            logger.debug("УПД имеет приоритет над счетом-фактурой")
            return "УПД"
        elif "ТОРГ-12" in found_types and "СЧЕТ_ФАКТУРА" in found_types:
            logger.debug("ТОРГ-12 имеет приоритет")
            return "ТОРГ-12"
        elif len(found_types) == 1:
            doc_type = found_types[0]
            logger.debug(f"Документ классифицирован как: {doc_type}")
            return doc_type
        else:
            logger.debug(f"Неоднозначность, возвращаем первый тип: {found_types[0]}")
